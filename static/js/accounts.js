...
async function handleDeposit(accountId) {
    ...
    // Show secondary confirmation/details screen instead of fetching immediately
    renderPaymentDetails(accountId, amount, mode);
}

function renderPaymentDetails(accountId, amount, mode) {
    const modalContent = window.currentModal.querySelector('div');
    
    let detailsHtml = '';
    
    const banks = ['SBI', 'HDFC', 'ICICI', 'AXIS', 'UNION', 'BOB', 'BOM'];
    ...
    if (mode === 'UPI') {
        detailsHtml = `
            <div class="payment-detail" style="text-align: center;">
                ...
                <div class="qr-code-placeholder" style="margin: 0 auto;"></div>
                <p style="margin-bottom: 10px;">Scan QR Code using any UPI App</p>
                <div class="countdown-timer" id="depositUpiTimer">02:00</div>
                <p>OR</p>
        ...
        detailsHtml = `
            <div class="payment-detail">
                <p style="margin-bottom: 15px; text-align: center;">Amount to pay: <strong>₹${amount}</strong></p>
                <div class="form-group">
                    <label>Select Bank</label>
                    <select id="nbBank" required>
        ...
        detailsHtml = `
            <div class="payment-detail">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>CVV</label>
                        <input type="password" id="cardCvv" placeholder="XXX" maxlength="3" required>
                    </div>
                </div>
            </div>
        `;
    ...
    modalContent.innerHTML = `
        <h3 style="margin-bottom: 20px; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px;">Confirm Deposit via ${mode}</h3>
        <form id="finalDepositForm">
            <div id="finalErrorMessage" class="error-message" style="display: none;"></div>
            <div id="finalSuccessMessage" class="success-message" style="display: none;"></div>
            
            ${detailsHtml}
            
            <div class="form-group" style="margin-top: 20px;">
                <button type="submit" class="btn-primary" id="confirmDepositBtn">Confirm & Pay</button>
                <button type="button" class="btn-secondary" onclick="closeDepositModal()">Cancel</button>
            </div>
        </form>
    `;
    ...
    if (mode === 'UPI') {
        startDepositTimer(120, document.getElementById('depositUpiTimer'));
    }

    document.getElementById('finalDepositForm').addEventListener('submit', function(e) {
        e.preventDefault();
        const btn = document.getElementById('confirmDepositBtn');
        btn.disabled = true;
        btn.textContent = 'Processing...';
        executeDeposit(accountId, amount, mode);
    });
}

function startDepositTimer(duration, display) {
    let timer = duration, minutes, seconds;
    if (depositCountdownInterval) clearInterval(depositCountdownInterval);
    depositCountdownInterval = setInterval(function () {
...
async function executeDeposit(accountId, amount, mode) {
    const errorMessage = document.getElementById('finalErrorMessage');
    ...
    try {
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                ...
                if (document.getElementById('accountsList')) {
                    loadAccounts();
                } else if (document.getElementById('allAccountsList')) {
                    loadAllAccounts();
                } else {
            ...
            document.getElementById('confirmDepositBtn').textContent = 'Confirm & Pay';
        }
    } catch (error) {
        errorMessage.textContent = 'Network error. Please try again.';
        errorMessage.style.display = 'block';
